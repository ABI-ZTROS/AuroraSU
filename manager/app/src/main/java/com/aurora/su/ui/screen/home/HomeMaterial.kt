package com.aurora.su.ui.screen.home
...
import androidx.compose.ui.unit.dp
import com.aurora.su.KernelVersion
import com.aurora.su.Natives
import com.aurora.su.R
import com.aurora.su.ksuApp
import com.topjohnwu.superuser.ShellUtils
import com.aurora.su.ui.component.dialog.rememberConfirmDialog
import com.aurora.su.ui.component.material.TonalCard
import com.aurora.su.ui.component.rebootlistpopup.RebootListPopup
import com.aurora.su.ui.component.statustag.StatusTag
            ...
            if (state.showRequireKernelWarning) {
                WarningCard(
                    stringResource(id = R.string.require_kernel_version,
                        state.ksuVersion ?: 0,
                        com.aurora.su.Natives.MINIMAL_SUPPORTED_KERNEL
                    )
                )
            }
...
private fun QuickActionsCard(
    ...
    var selinuxStatus by remember { mutableStateOf(state.systemInfo.selinuxStatus) }
    var suEnabled by remember { mutableStateOf(Natives.isSuEnabled()) }
    var selinuxHideEnabled by remember { mutableStateOf(Natives.isSelinuxHideEnabled()) }
    var kernelUmountEnabled by remember { mutableStateOf(Natives.isKernelUmountEnabled()) }

    Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
        // Row 1: SELinux toggle + Safe mode toggle
        Row(
            ...
            TonalCard(
                ...
                onClick = {
                    val isEnforcing = selinuxStatus == "Enforcing"
                    val result = ShellUtils.fastCmdResult("su -c setenforce ${if (isEnforcing) 0 else 1}")
                    if (result) {
                        selinuxStatus = if (isEnforcing) "Permissive" else "Enforcing"
                        Toast.makeText(
                            ksuApp,
                            if (isEnforcing) "SELinux: Permissive" else "SELinux: Enforcing",
                            Toast.LENGTH_SHORT
                        ).show()
                    } else {
                        Toast.makeText(ksuApp, "SELinux toggle failed", Toast.LENGTH_SHORT).show()
                    }
                }
            ) {
                Column(
                    ...
                    Text(
                        ...
                        color = if (selinuxStatus == "Enforcing")
            ...
            TonalCard(
                modifier = Modifier.weight(1f),
                onClick = {
                    val newEnabled = !suEnabled
                    val result = Natives.setSuEnabled(newEnabled)
                    if (result) {
                        suEnabled = newEnabled
                        Toast.makeText(
                            ksuApp,
                            if (newEnabled) "Safe mode: Disabled" else "Safe mode: Enabled",
                            Toast.LENGTH_SHORT
                        ).show()
                    } else {
                        Toast.makeText(ksuApp, "Safe mode toggle failed", Toast.LENGTH_SHORT).show()
                    }
                }
            ) {
                Column(
                    ...
                    Text(
                        text = if (suEnabled) "Disabled" else "Enabled",
                        style = MaterialTheme.typography.bodyMedium,
                        color = if (suEnabled)
                            MaterialTheme.colorScheme.primary
                        else
        ...
        Row(
            ...
            TonalCard(
                modifier = Modifier.weight(1f),
                onClick = {
                    val newEnabled = !selinuxHideEnabled
                    val result = Natives.setSelinuxHideEnabled(newEnabled)
                    if (result == 0) {
                        selinuxHideEnabled = newEnabled
                        Toast.makeText(
                            ksuApp,
                            if (newEnabled) "Root hide: Enabled (reboot required)" else "Root hide: Disabled",
                            Toast.LENGTH_SHORT
                        ).show()
                    } else {
                        Toast.makeText(ksuApp, "Root hide toggle failed", Toast.LENGTH_SHORT).show()
                    }
                }
            ) {
                Column(
                    ...
                    Text(
                        text = if (selinuxHideEnabled) "Enabled" else "Disabled",
                        style = MaterialTheme.typography.bodyMedium,
                        color = if (selinuxHideEnabled)
                            MaterialTheme.colorScheme.primary
                        else
                            MaterialTheme.colorScheme.error
                    )
            ...
            // Kernel umount toggle
            TonalCard(
                modifier = Modifier.weight(1f),
                onClick = {
                    val newEnabled = !kernelUmountEnabled
                    val result = Natives.setKernelUmountEnabled(newEnabled)
                    if (result) {
                        kernelUmountEnabled = newEnabled
                        Toast.makeText(
                            ksuApp,
                            if (newEnabled) "Kernel umount: Enabled" else "Kernel umount: Disabled",
                            Toast.LENGTH_SHORT
                        ).show()
                    } else {
                        Toast.makeText(ksuApp, "Kernel umount toggle failed", Toast.LENGTH_SHORT).show()
                    }
                }
            ) {
                Column(
                    ...
                    Icon(
                        ...
                        contentDescription = "Kernel umount",
                        modifier = Modifier.padding(bottom = 4.dp)
                    )
                    Text(
                        text = "Kernel umount",
                    ...
                    Text(
                        text = if (kernelUmountEnabled) "Enabled" else "Disabled",
                        style = MaterialTheme.typography.bodyMedium,
                        color = if (kernelUmountEnabled)
                            MaterialTheme.colorScheme.primary
                        else
                            MaterialTheme.colorScheme.error
                    )
...
private fun previewHomeScreenState(
    ...
    latestVersionInfo = com.aurora.su.ui.util.module.LatestVersionInfo(),
